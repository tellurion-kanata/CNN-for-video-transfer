...
class videoNet(BaseModel):
    ...
    def backward(self):
        ...
        self.content_loss *= self.opt.lambda_content
        self.style_loss   *= self.opt.lambda_style
        self.tv_loss      *= self.opt.lambda_tv
    ...
    def train(self):
        ...
        for epoch in range(self.st_epoch, self.ed_epoch):
            for idx, data in enumerate(self.datasets):
                ...
                T, c, h, w = self.frames.shape
                self.prev_x = self.frames[0].cuda().unsqueeze(0)
